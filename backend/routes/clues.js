...
const { activePhaseOnly } = require("../middleware/phaseMiddleware");
...
router.post("/buy-clue", verifyFirebaseToken, activePhaseOnly, async (req, res, next) => {
...
router.post("/submit", verifyFirebaseToken, activePhaseOnly, async (req, res, next) => {
  ...
  try {
    ...
    if (isCorrect) {
      ...
      if (!alreadyCracked) {
        ...
        const updateResult = await Team.updateOne(
          { _id: team._id, "crackedAccounts.username": { $ne: account.username } },
          {
...
router.post("/inject-fake-clue", verifyFirebaseToken, activePhaseOnly, async (req, res, next) => {
