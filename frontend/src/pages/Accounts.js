...
import { getAccounts, verifyAccountPassword, injectFakeClue, getLeaderboard, getGameState } from "../services/api";
...
export default function Accounts() {
  ...
  useEffect(() => {
    ...
    getGameState().then((res) => setPhase(res.data.phase)).catch(() => {});
  ...
  const handlePasswordSubmit = async () => {
    ...
    if (phase !== "recon" && phase !== "chaos") {
      toast.error(`Access Denied: Game is ${phase === "paused" ? "PAUSED" : phase === "ended" ? "ENDED" : "not started yet"}`);
      return;
    }
              ...
              <div className="terminal-body">
                ...
                >
                  ...
                  <button 
                    ...
                    type="submit" 
                    disabled={cooldownLeft > 0 || (phase !== "recon" && phase !== "chaos")} 
                    style={{ 
                      opacity: (cooldownLeft > 0 || (phase !== "recon" && phase !== "chaos")) ? 0.5 : 1, 
                      cursor: (cooldownLeft > 0 || (phase !== "recon" && phase !== "chaos")) ? "not-allowed" : "pointer" 
                    }}
                  >
                    {phase === "paused" ? "PAUSED" : phase === "ended" ? "ENDED" : "EXECUTE"}
                  </button>
                </form>
              ...
              <div className="terminal-body">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 10 }}>
                  ...
                  <button className="btn btn-primary" 
                    disabled={phase !== "recon" && phase !== "chaos"}
                    onClick={async () => {
                    if (phase !== "recon" && phase !== "chaos") return toast.error("Game is not active");
