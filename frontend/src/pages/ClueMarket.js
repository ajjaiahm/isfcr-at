...
import { buyClue as buyClueRequest, getClues, getGameState } from "../services/api";
...
export default function ClueMarket() {
  ...
  useEffect(() => {
    ...
    loadClues();
    loadPhase();
    const intv = setInterval(loadPhase, 10000);
          ...
          {clues.map((c) => {
            ...
            return (
              <div key={c._id || c.id} className="clue-card">
                ...
                <button
                  className={`btn ${canBuy && (phase === "recon" || phase === "chaos") ? "btn-primary" : "btn-ghost"}`}
                  onClick={() => buyClue(c)}
                  disabled={!canBuy || (phase !== "recon" && phase !== "chaos")}
                  style={{ 
                    width: "100%", 
                    opacity: (canBuy && (phase === "recon" || phase === "chaos")) ? 1 : 0.7,
                    cursor: (phase !== "recon" && phase !== "chaos") ? "not-allowed" : "pointer" 
                  }}
                >
                  {phase === "paused" ? "PAUSED" : phase === "ended" ? "ENDED" : (canBuy ? "Buy Clue" : "Not enough coins")}
                </button>
              </div>
            );
          })}
